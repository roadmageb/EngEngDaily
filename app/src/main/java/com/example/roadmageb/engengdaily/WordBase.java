...
public class WordBase extends AppCompatActivity {
    ...
    public WordBase () {
        origin = "abacus\tframe with balls for calculating\n" +
        ...
        for(String wordmean : origin.split("\n")){
    ...
    public void updateWordStat(String word, boolean ox){
        for(WordStat i : wordStat){
            if(i.word == word) {
                if(ox)
                    i.stat/=2;
                else
                    i.stat = Math.min(30,i.stat+5);
                return;
            }
        }
    }
    public void setWordStat(String word, int stat){_setWordStat(word, stat);}
    public void setWordStat(String word){_setWordStat(word, 20);}
    void _setWordStat(String word, int stat){
        for(WordStat i : wordStat){
            if(i.word == word) {
                i.stat = stat;
                return;
        ...
        wordStat.add(new WordStat(word,stat));
    }
    public void loadStat() {
        try{
            BufferedReader br = new BufferedReader(new FileReader(SelectActivity.masterDir + "stat.txt"));
            String line;
    ...
    public void saveStat() {
        try{
            BufferedWriter bw = new BufferedWriter(new FileWriter(SelectActivity.masterDir + "stat.txt",false));
    ...
    public void loadProgress() {
        try{
            BufferedReader br = new BufferedReader(new FileReader(SelectActivity.masterDir + "progress.txt"));
            String line = br.readLine();
            Date date = new Date();
            SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
            if(sdf.format(date).equals(line)){
                testProgress = Integer.parseInt(br.readLine());
                reciteEnd = Integer.parseInt(br.readLine());
            ...
            testProgress = 0;
            reciteEnd = 0;
            for(int i=0; todayWord.size()<10 && unWord.size()>0; i++){
                if(wordDB.isEmpty()) break;
                int tmp = random.nextInt(unWord.size());
                todayWord.add(unWord.get(tmp));
                unWord.remove(tmp);
    ...
    public void saveProgress() {
        try{
            BufferedWriter bw = new BufferedWriter(new FileWriter(SelectActivity.masterDir + "progress.txt",false));
            SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
            bw.write(sdf.format(new Date())+"\n"+testProgress+"\n"+reciteEnd+"\n");
            bw.write(todayWord.size()+"\n");
            for(int i=0;i<todayWord.size();i++)
                bw.write(todayWord.get(i)+"\n");
            bw.write(testList.size()+"\n");
            for(int i=0;i<testList.size();i++)
                bw.write(testList.get(i)+"\n");
            bw.close();
        }catch(Exception e){
